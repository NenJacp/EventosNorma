...
import { Menu, X, LayoutDashboard, Calendar, Users, Globe, MapPin, Tag, List, LogOut, Shield } from "lucide-react";
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        const res = await apiFetch<AdminUser>("/api/Users/currentUser");
        if (res.role !== "Admin") {
          router.push("/home");
          return;
        }
    ...
    <div className="min-h-screen flex bg-slate-100 text-slate-900 font-sans">
      ...
      <aside className={`fixed inset-y-0 left-0 w-64 bg-[#0f172a] text-slate-400 transform transition-transform duration-200 z-40 flex flex-col ${isOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 ${sidebarVisible ? "md:translate-x-0" : "md:-translate-x-full"}`}>
        ...
        <div className="flex-1 overflow-y-auto py-6 space-y-6">
          {adminMenuSections.map((section) => (
            <div key={section.title}>
              <h3 className="px-6 mb-3 text-[11px] font-semibold text-slate-500 tracking-wider">{section.title}</h3>
                  ...
                  return (
                    <li key={item.href}>
                      <Link href={item.href} onClick={() => setIsOpen(false)}
                        className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors ${isActive ? "bg-[#1e293b] text-white relative before:absolute before:left-[-12px] before:top-0 before:bottom-0 before:w-1 before:bg-amber-500" : "hover:bg-white/5 hover:text-white"}`}>
        ...
        <div className="border-t border-white/5 p-4 bg-[#0b1121]">
          <Link href="/home" onClick={() => setIsOpen(false)} className="flex items-center gap-3 px-2 mb-4 hover:bg-white/5 rounded-lg p-1 -mx-1 transition-colors">
            <div className="w-10 h-10 rounded-full bg-amber-500 flex items-center justify-center text-white font-semibold text-sm">{initials}</div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-white truncate">{fullName}</p>
              <p className="text-[11px] text-amber-400 truncate">Administrador</p>
            </div>
          </Link>
          <div className="flex gap-2">
            <Link href="/home" className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-slate-700 text-slate-300 hover:bg-slate-600 transition-colors text-sm font-medium">
              Ver Sitio
            </Link>
            <button onClick={handleLogout} disabled={loadingLogout} className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg border border-white/10 text-slate-300 hover:bg-white/5 transition-colors text-sm font-medium">
              <LogOut size={16} />
            </button>
          </div>
        </div>
