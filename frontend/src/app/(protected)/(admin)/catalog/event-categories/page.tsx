...
import { List, Search, Edit2, ToggleLeft, ToggleRight, Plus, X } from "lucide-react";
...
export default function EventCategoriesPage() {
  ...
  const [editingItem, setEditingItem] = useState<EventCategoryItem | null>(null);
  ...
  const fetchItems = useCallback(async () => {
    setLoading(true);
    try {
      const data = await apiFetch<EventCategoryItem[]>("/api/EventCategories?includeInactive=true");
      setItems(data);
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem) {
        await apiFetch(`/api/EventCategories/${editingItem.id}`, { method: "PUT", body: JSON.stringify({ id: editingItem.id, name: formData.name, description: formData.description || null, isActive: editingItem.isActive }) });
      } else {
        await apiFetch("/api/EventCategories", { method: "POST", body: JSON.stringify({ name: formData.name, description: formData.description || null }) });
      }
  ...
  const handleToggle = async (id: number) => {
    try {
      await apiFetch(`/api/EventCategories/${id}/toggle`, { method: "POST" });
      fetchItems();
    } catch (err) {
  ...
  return (
    <div>
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Categorías de Evento</h1>
          <p className="text-sm text-slate-500">Gestión de categorías de evento</p>
        </div>
        <div className="flex items-center gap-4">
          ...
          <button onClick={() => { setEditingItem(null); setFormData({ name: "", description: "" }); setIsModalOpen(true); }}
          ...
          <table className="w-full">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase">Eventos</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredItems.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-6 py-4 text-sm text-slate-600">{item.eventsCount}</td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <button onClick={() => { setEditingItem(item); setFormData({ name: item.name, description: item.description || "" }); setIsModalOpen(true); }}
          ...
          <div className="fixed inset-0 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
              <div className="flex items-center justify-between px-6 py-4 border-b">
                <h2 className="text-lg font-semibold">{editingItem ? "Editar" : "Nueva"} Categoría</h2>
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} rows={3} className="w-full px-3 py-2 border border-slate-300 rounded-lg resize-none" />
                </div>
