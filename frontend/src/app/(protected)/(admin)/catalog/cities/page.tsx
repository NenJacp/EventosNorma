...
export default function CitiesPage() {
  ...
  const [formData, setFormData] = useState({ name: "", code: "", stateId: "" });
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      const [citiesData, statesData] = await Promise.all([
        apiFetch<CityItem[]>("/api/Cities?includeInactive=true"),
        apiFetch<{ id: number; name: string }[]>("/api/States"),
      ]);
      setItems(citiesData);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem) {
        await apiFetch(`/api/Cities/${editingItem.id}`, { method: "PUT", body: JSON.stringify({ id: editingItem.id, name: formData.name, code: formData.code || null, stateId: parseInt(formData.stateId), isActive: editingItem.isActive }) });
      } else {
        await apiFetch("/api/Cities", { method: "POST", body: JSON.stringify({ name: formData.name, code: formData.code || null, stateId: parseInt(formData.stateId) }) });
      }
  ...
  const handleToggle = async (id: number) => {
    try {
      await apiFetch(`/api/Cities/${id}/toggle`, { method: "POST" });
      fetchItems();
    } catch (err) {
    ...
    <div>
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500">Gestión de ciudades</p>
        </div>
        <div className="flex items-center gap-4">
          ...
          <button onClick={() => { setEditingItem(null); setFormData({ name: "", code: "", stateId: "" }); setIsModalOpen(true); }}
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredItems.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-6 py-4 text-sm text-slate-600">{item.stateName}</td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <button onClick={() => { setEditingItem(item); setFormData({ name: item.name, code: item.code || "", stateId: item.stateId.toString() }); setIsModalOpen(true); }}
          ...
          <div className="fixed inset-0 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
              <div className="flex items-center justify-between px-6 py-4 border-b">
                <h2 className="text-lg font-semibold">{editingItem ? "Editar" : "Nueva"} Ciudad</h2>
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <select required value={formData.stateId} onChange={(e) => setFormData({ ...formData, stateId: e.target.value })} className="w-full px-3 py-2 border border-slate-300 rounded-lg">
