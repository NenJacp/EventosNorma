...
import { MapPin, Search, Edit2, ToggleLeft, ToggleRight, Plus, X } from "lucide-react";
...
export default function StatesPage() {
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      const [statesData, countriesData] = await Promise.all([
        apiFetch<StateItem[]>("/api/States?includeInactive=true"),
        apiFetch<{ id: number; name: string }[]>("/api/Countries"),
      ]);
      setItems(statesData);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem) {
        await apiFetch(`/api/States/${editingItem.id}`, { method: "PUT", body: JSON.stringify({ id: editingItem.id, name: formData.name, code: formData.code || null, countryId: parseInt(formData.countryId), isActive: editingItem.isActive }) });
      } else {
        await apiFetch("/api/States", { method: "POST", body: JSON.stringify({ name: formData.name, code: formData.code || null, countryId: parseInt(formData.countryId) }) });
      }
  ...
  const handleToggle = async (id: number) => {
    try {
      await apiFetch(`/api/States/${id}/toggle`, { method: "POST" });
      fetchItems();
    } catch (err) {
  ...
  return (
    <div>
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Estados</h1>
          <p className="text-sm text-slate-500">Gestión de estados/regiones</p>
        </div>
        <div className="flex items-center gap-4">
          ...
          <button onClick={() => { setEditingItem(null); setFormData({ name: "", code: "", countryId: "" }); setIsModalOpen(true); }}
          ...
          <table className="w-full">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase">Ciudades</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredItems.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-6 py-4 text-sm text-slate-600">{item.countryName}</td>
                  <td className="px-6 py-4 text-sm text-slate-600">{item.citiesCount}</td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <button onClick={() => { setEditingItem(item); setFormData({ name: item.name, code: item.code || "", countryId: item.countryId.toString() }); setIsModalOpen(true); }}
          ...
          <div className="fixed inset-0 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
              <div className="flex items-center justify-between px-6 py-4 border-b">
                <h2 className="text-lg font-semibold">{editingItem ? "Editar" : "Nuevo"} Estado</h2>
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <select required value={formData.countryId} onChange={(e) => setFormData({ ...formData, countryId: e.target.value })} className="w-full px-3 py-2 border border-slate-300 rounded-lg">
