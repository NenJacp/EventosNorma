...
import { Tag, Search, Edit2, ToggleLeft, ToggleRight, Plus, X } from "lucide-react";
...
export default function EventTypesPage() {
  ...
  const fetchItems = useCallback(async () => {
    setLoading(true);
    try {
      const data = await apiFetch<EventTypeItem[]>("/api/EventTypes?includeInactive=true");
      setItems(data);
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem) {
        await apiFetch(`/api/EventTypes/${editingItem.id}`, { method: "PUT", body: JSON.stringify({ id: editingItem.id, name: formData.name, description: formData.description || null, isActive: editingItem.isActive }) });
      } else {
        await apiFetch("/api/EventTypes", { method: "POST", body: JSON.stringify({ name: formData.name, description: formData.description || null }) });
      }
  ...
  const handleToggle = async (id: number) => {
    try {
      await apiFetch(`/api/EventTypes/${id}/toggle`, { method: "POST" });
      fetchItems();
    } catch (err) {
    ...
    <div>
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500">Gestión de tipos de evento</p>
        </div>
          ...
          <div className="fixed inset-0 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
              <div className="flex items-center justify-between px-6 py-4 border-b">
                <h2 className="text-lg font-semibold">{editingItem ? "Editar" : "Nuevo"} Tipo de Evento</h2>
