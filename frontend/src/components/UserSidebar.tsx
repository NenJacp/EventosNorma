...
export default function UserSidebar({ isOpen, onToggle, visible }: UserSidebarProps) {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        const res = await apiFetch<UserData>("/api/Users/currentUser");
        setUser(res);
      } catch {
        router.push("/login");
      }
      ...
      <div className="border-t border-white/5 p-4 bg-[#0b1121]">
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] text-slate-400 truncate">Usuario</p>
          </div>
        </Link>
